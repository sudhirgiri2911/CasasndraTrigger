...
public class CustomCommitLogReadHandler implements CommitLogReadHandler {
    ...
    private final String keyspace = "practice";
    private final String table = "movies_by_genre";
    ...
    private void process(Partition partition) {
        ...
        if (!partition.metadata().ksName.equals(keyspace)) {
            System.out.println("Keyspace should be '{}' but is '{}'.", keyspace, partition.metadata().ksName);
            return;
        }
        ...
        LOGGER.debug("Created producer record with key {}, value {}", key, value);
